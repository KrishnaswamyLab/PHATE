...
def test_phate_numpy_array():
    ...
    ), f"Expected numpy.ndarray output, got {type(embedding)}"
...
def test_phate_scipy_sparse_csr():
    ...
    print(
        f"Sparsity: {1 - sparse_data.nnz / (sparse_data.shape[0] * sparse_data.shape[1]):.2%}"
    )
    ...
    assert embedding.shape == (
        sparse_data.shape[0],
        2,
    ), f"Expected shape ({sparse_data.shape[0]}, 2), got {embedding.shape}"
    assert np.all(np.isfinite(embedding)), "Embedding contains non-finite values"
    assert phate_op.graph is not None, "Graph not created"
...
def test_phate_pandas_dataframe():
    ...
    assert embedding.shape == (
        df.shape[0],
        2,
    ), f"Expected shape ({df.shape[0]}, 2), got {embedding.shape}"
    assert np.all(np.isfinite(embedding)), "Embedding contains non-finite values"
    assert phate_op.graph is not None, "Graph not created"
...
def test_phate_anndata():
    ...
    assert embedding.shape == (
        adata.shape[0],
        2,
    ), f"Expected shape ({adata.shape[0]}, 2), got {embedding.shape}"
    assert np.all(np.isfinite(embedding)), "Embedding contains non-finite values"
    assert phate_op.graph is not None, "Graph not created"
...
def test_consistency_across_input_types():
    ...
    if PANDAS_AVAILABLE:
        ...
        assert compare_embeddings(
            embedding_numpy, embedding_df, rtol=1e-4, atol=1e-6
    ...
    if ANNDATA_AVAILABLE:
        ...
        assert compare_embeddings(
            embedding_numpy, embedding_adata, rtol=1e-4, atol=1e-6
