...
def test_no_eigmin_eigmax(n_points, symmetric, jackson, cumulative):
    ...
    sample_points, density_est = phate.eig.estimate_eigenvalue_density(A, 
        n_points=n_points, cumulative=cumulative, jackson=jackson, symmetric=symmetric)
    ...
    assert np.mean(density - np.round(density_est) <= 1) > 0.5, np.mean(density - np.round(density_est) <= 1)
    assert np.mean(density - np.round(density_est) <= 2) > 0.95, np.mean(density - np.round(density_est) <= 2)
...
def test_eigmin_eigmax(n_points, symmetric, jackson, cumulative):
    ...
    if not symmetric:
        warnings.filterwarnings("ignore", "Casting complex values to real discards the imaginary part")
    sample_points_est, density_est = phate.eig.estimate_eigenvalue_density(A, 
        n_points=n_points, cumulative=cumulative, jackson=jackson, symmetric=symmetric, eigmin=np.min(eigs), eigmax=np.max(eigs))
    warnings.resetwarnings()
...
def test_order(order, atol, jackson, seed, n_points=100, cumulative=False, symmetric=True):
    np.random.seed(seed)
    data = np.random.uniform(0, 1, (100, 100))
    G = graphtools.Graph(data)
    A = G.diff_aff if symmetric else G.diff_op
    ...
    if not symmetric:
        warnings.filterwarnings("ignore", "Casting complex values to real discards the imaginary part")
    sample_points, density_est = phate.eig.estimate_eigenvalue_density(A, 
        n_points=n_points, cumulative=cumulative, jackson=jackson, symmetric=symmetric, order=order)
    warnings.resetwarnings()
    density = np.array([np.sum(eigs <= s) for s in sample_points])
    if not cumulative:
        density[1:] -= density[:-1].copy()
